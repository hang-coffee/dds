...
Expr* Parser::parse_sizeof() {
	...
	if (is(TOK_LPAREN)) {
		...
		advance();
		// 尝试解析为类型
		if (is(TOK_INT) || is(TOK_SHORT) || is(TOK_LONG) || is(TOK_CHAR) || is(TOK_VOID) ||
	...
	for (;;) {
		...
		if (accept(TOK_ASSIGN)) {
			...
			if (is(TOK_STRING)) {
				...
				Expr* e = parse_assign();
				if (e && e->kind == E_INT) gv.ival_init = e->ival;
...
Program Parser::parse() {
	Program prog;
	while (!is(TOK_EOF)) {
		if (is(TOK_EXTERN) || is(TOK_INT) || is(TOK_SHORT) || is(TOK_LONG) || is(TOK_CHAR) || is(TOK_VOID) ||
		    is(TOK_SIGNED) || is(TOK_UNSIGNED) || is(TOK_CONST) || is(TOK_STRUCT) || is(TOK_UNION) ||
		    is(TOK_ENUM) || is(TOK_TYPEDEF) || is(TOK_IDENT)) parse_global(prog);
...
Stmt* Parser::parse_stmt() {
	...
	if (is(TOK_INT) || is(TOK_SHORT) || is(TOK_LONG) || is(TOK_CHAR) || is(TOK_VOID) || is(TOK_SIGNED) || is(TOK_UNSIGNED) || is(TOK_CONST) ||
...
Stmt* Parser::parse_decl_stmt(Type t, const std::string& name) {
...
bool Parser::is_cast_start() const {
	...
	if (k == TOK_INT || k == TOK_SHORT || k == TOK_LONG || k == TOK_CHAR || k == TOK_VOID ||
...
Expr* Parser::parse_unary() {
	...
	if (is(TOK_STAR)) {		// *p：解引用
		...
		e->op = "*";
		e->r = parse_unary();
		if (e->r && e->r->type.is_ptr()) e->type = e->r->type.pointee();
...
Expr* Parser::parse_postfix() {
	...
	for (;;) {
		...
		if (is(TOK_LPAREN)) {		// 调用
			...
			n->kind = E_CALL;
			n->name = e->name;
			n->type = type_int();	// 默认；codegen 会查函数表修正
			if (!is(TOK_RPAREN)) {
				for (;;) {
			...
			if (!expect(TOK_RPAREN, "')'")) { delete e; delete n; return nullptr; }
			delete e;
			e = n;
			continue;
		}
...
Expr* Parser::parse_primary() {
	...
	if (is(TOK_IDENT)) {
		...
		// __reg_ 前缀 → 寄存器直访（A/B/C/D1/D2/X/I）
