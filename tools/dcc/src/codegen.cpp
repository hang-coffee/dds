...
std::string CodeGen::generate(Program& prog, const TypeEnv& env) {
	...
	// 入口 _start 必须在 TEXT 段最前（模拟器从 P=0 执行）
	gen_start();
	// 函数 → TEXT
...
void CodeGen::gen_func(Function& f) {
	...
	if (f.is_isr) {
		...
		if (!cur_ret_long_) emit_t("MOV D1, A");
		emit_t("RER");
		emit_t("JMP");
	}
...
Type CodeGen::resolve_type(Expr* e) {
	...
	switch (e->kind) {
		...
		case E_UNOP:
			...
			if (e->op == "*") return resolve_type(e->r).pointee();
			if (e->op == "-" || e->op == "~") {
		...
		case E_COND:
			return resolve_type(e->l);
		case E_CALL: {
			// 函数返回类型（parser 的 e->type 是 int 占位，查函数表修正）
...
void CodeGen::gen_lvalue_addr(Expr* e) {
	...
	if (e->kind == E_VAR) {
		const Symbol* s = symtab_.lookup(e->name);
		if (!s) { error("左值未定义: " + e->name); emit_t("LET B, DWORD 0"); return; }
...
void CodeGen::gen_expr(Expr* e) {
	...
	switch (e->kind) {
		...
		case E_VAR: {
			const Symbol* s = symtab_.lookup(e->name);
			if (!s) { error("未定义变量: " + e->name); emit_t("LET A, DWORD 0"); break; }
			if (s->is_array) {	// 数组名 → 基址
				gen_var_addr_to_a(*s);
				break;
			}
...
void CodeGen::gen_call(Expr* e) {
	...
	emit_t("MOV A, F");
	emit_t("PUSH DWORD A");
	// 参数类型（查函数表；未找到则用实参表达式类型）
	const Function* fn = nullptr;
	auto fit = funcs_.find("func_" + e->name);
	if (fit != funcs_.end()) fn = fit->second;
	int argbytes = 0;
	for (size_t i = 0; i < e->args.size(); i++) {
		Expr* a = e->args[i];
		gen_expr(a);
		Type pt = (fn && i < fn->params.size()) ? fn->params[i] : resolve_type(a);
		if (pt.is_long()) {
			// long 参数：int 实参先提升为 long
			Type at = resolve_type(a);
			if (!at.is_long()) {
				if (at.is_unsigned_scalar()) emit_t("ZERO D1");
				else emit_d1_signext();
			}
	...
	emit_t("PUSH DWORD E");
	// 跳转
	std::string fn2 = "func_" + e->name;
	if (funcs_.find(fn2) == funcs_.end()) error("未定义函数: " + e->name);
	...
	auto rit = func_ret_.find(fn2);
	bool ret_long = (rit != func_ret_.end()) && rit->second.is_long();
	if (!ret_long) emit_t("MOV A, D1");
}
