...
cat > "$T/f1.das" <<'EOF'
	...
	HLT
EOF
$DASM "$T/f1.das" "$T/f1.code" "$T/f1.data" >/dev/null 2>&1
H=$(hexstr "$T/f1.code")
...
cat > "$T/f2.das" <<'EOF'
	...
	HLT
EOF
$DASM "$T/f2.das" "$T/f2.code" "$T/f2.data" >/dev/null 2>&1
H=$(hexstr "$T/f2.code")
...
cat > "$T/f3.das" <<'EOF'
	...
	HLT
EOF
$DASM "$T/f3.das" "$T/f3.code" "$T/f3.data" >/dev/null 2>&1
H=$(hexstr "$T/f3.code")
...
LAB:
	...
	HLT
EOF
$DASM "$T/f4.das" "$T/f4.code" "$T/f4.data" >/dev/null 2>&1
H=$(hexstr "$T/f4.code")
...
MSG:
	DB 0x228, "AB"
EOF
$DASM "$T/f5.das" "$T/f5.code" "$T/f5.data" >/dev/null 2>&1
H=$(hexstr "$T/f5.data")
...
$DASM "$T/f5.das" "$T/f5.code" "$T/f5.data" 2>&1 | grep -q "MSG.*0x222" \
...
NEXT:
	...
	HLT
EOF
$DASM "$T/f6.das" "$T/f6.code" "$T/f6.data" >/dev/null 2>&1
H=$(hexstr "$T/f6.code")
assert_substr "RER 编码为 00 19 (2字节)" "$H" "0019"
$DASM "$T/f6.das" "$T/f6.code" "$T/f6.data" 2>&1 | grep -q "NEXT.*0x2" \
...
; 更多注释
	...
	BADINSTR B
EOF
assert_err_line "错误行号 = 源文件行号 (BADINSTR 在第8行)" "$T/f7.das" 8
