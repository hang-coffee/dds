...
static int is_keyword(const char *s) {
    static const char *kw[] = {
        ...
        "float", "double", "_Bool", "volatile", "restrict", "struct",
        "union", "enum", "typedef", "static", "extern", "inline", "__interrupt__",
        "return", "if", "else", "while", "for", "break", "continue", "do", "goto", "sizeof",
...
TokenArray tokenize(const char *src) {
    ...
    while (src[i]) {
        ...
        fprintf(stderr, "dcc: line %d: 无法识别的字符 '%c'\n", line, c);
        i++;
    }
    Token eof;
