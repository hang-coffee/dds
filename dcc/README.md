...
`dcc` 是为 **DOCTOR ISA v3.3** 编写的 C 编译器（v0.2），把 **ANSI C 的可用于集** 编译为 **dasm 汇编文本**，再经 `dasm` 汇编成二进制，由 `doctor-sim` 模拟器执行。
...
- **多文件分离编译**：`dcc main.c lib.c out.asm` —— 每个 `.c` 独立预处理
  ...
  `dcc -m elf main.c lib.c out.o` 则直接输出合并后的 ELF 目标文件。
  声明放头文件，实现在 `.c`：
  ```c
  ...
  ```
- 最后一个参数以 `.asm` / `.bin` / `.o` 结尾视为输出；也可用 `-o` 显式指定。否则默认 `<第一个输入去扩展名>.asm`（`-m elf` 时为 `.o`）。
...
dcc 不再自动生成 `_start` 入口。默认入口符号为 `func_main`，由用户自行链接 CRT：
...
sh run_elf_test.sh            # 运行 -m elf 冒烟测试
make test-elf                 # 等价于 sh run_elf_test.sh
```

每个测试源文件头注释声明期望值（`/* expect A = 0x.. */`），脚本执行
`dcc → dasm → doctor-sim` 全链路并断言 A 寄存器。当前 43 个测试覆盖：
...
│   └── stdarg.h        # 可变参数支持（va_list/va_start/va_arg/va_end/va_copy）
...
- **类型**：`int`（4 字节，默认有符号）、`unsigned int`、`short`（2 字节）、
  ...
  `signed char`、`unsigned char`、**`_Bool`**（1 字节，C99 布尔，只存 0/1）、
  **`float`**（4 字节，DFE）、**`double`** / **`long double`**（8 字节，DDE）、**`void *`**（通用指针）、
...
- **`void *`**：通用指针（与其它指针互转需显式转换）；支持解引用（按 DWORD 访问）
...
- **数组初始化列表**：支持 `int a[3] = {1,2,3}` 与省略长度的 `int a[] = {1,2,3}`
- **无 `void` 指针**；`void` 仅用于函数返回类型或 `(void)` 空参数列表
- **移位右操作数必须是常量**
- **裸 `char` 按无符号处理**（0-255）；`signed char` 才是有符号
- **`__reg_` 寄存器直访**：支持 `A/B/C/D1/D2/X/I/S/R/F/T`；`E` 只能作右值。
  类型为 `unsigned int`；寄存器无地址（`&__reg_A` 报错）。
...
- **类型转换**：`(char)x` 截断低 8 位；int/unsigned/指针互转仅重解释、
  不改变位模式；不支持结构体转换
- **指针**：支持**函数指针**的声明、赋值、传参与间接调用（`fp(...)` 与 `(*fp)(...)`）；
  不支持 `void*`、指针与整数的隐式互转
...
- **分离编译**：支持多文件合并（`dcc a.c b.c out.asm`）、函数原型、`extern`
  ...
  编译器自动从 **`项目根/dcc/lib/foo.c`** 加载实现（如 `<io.h>` →
  `lib/io.c`、`<calc.h>` → `lib/calc.c`）；用户显式提供的实现优先，防循环
  依赖；**支持** `static` 文件内函数/变量（内部链接，多文件下按编译单元隔离）与 `static` 局部变量（静态存储期）；
  不支持动态链接（未定义函数/变量报错）
- **函数指针**：支持 `ret (*name)(params)`、`typedef` 函数指针、函数名/`&func` 作为地址、
  通过函数指针间接调用；不支持函数指针数组、返回函数指针的复杂声明符（可用 typedef 间接表达）
- **可变参数**：支持 `...` 与 `stdarg.h`；可变参数槽位统一按 8 字节传递，支持 `int`/指针/`char*`/`long`/`double` 等
- **局部变量总大小受栈空间限制**；栈顶固定 `0x300000`（避免与低地址数据区冲突）

### 预处理器 `#include` / `#define`

```c
#include <mylib.h>          /* 默认查找: 项目根/dcc/include */
...
- `#include <foo.h>` 默认在 **`项目根/dcc/include`** 查找
...
- dasm 预处理会在**字符串内**剥离 `;` 后的内容；dcc 生成的数据区字符串
  一律以十六进制字节输出以规避此限制
- 生成的代码不再包含 `_start`；默认入口符号为 `func_main`，由用户自行链接
