...
软件中断由`INT [N/DR]` 触发，行为与硬件中断一致。当全局中断关闭（GIE=0或在PUSHI/POPI对的内部）时，不允许调用软件中断。

全局中断关闭的两个条件为：在PUSHI/POPI内部，或GIE为0。前者优先级比后者高。
...
|---|---|---|---|
| `0x00` | 除数为0 | #DIV | 触发该异常时的P值 |
| `0x01` | 非法指令 | #II | 同上 |
| `0x02` | 栈异常，显式修改S或F越界，或是栈溢出 | #STACK | 最高位：0=上溢，1=下溢；低31位：导致溢出的S新值 |
| `0x03` | 一般保护异常，非S或F的指针越界、跳转目标非法、用户态执行特权指令 | #GP | 触发该异常时的P值 |
...
| 10 | `DIV QWORD` | `DIV QWORD` | D1:D2 / A:B → D1=商, D2=余数 | `0x0A` |
...
| `STO` | `0x1F` | 写入*R时 | 同上 |
| `PUSH` | `0x16` | 压栈时 | 检查S+尺寸是否<DLIMIT |
| `POP` | `0x17` | 弹栈时 | 检查S-尺寸是否>=DBASE |
| `BLKS` | `0x35` | 批量赋值时 | 遍历检查[R, R+C*尺寸] 是否越界 |
| `BLKOUT` | `0x3F` | 写入内存时 | 同上 |

**对于下述指令，MPU会检查内存指针是否越界：**
| 指令 | 操作码 | 何时检查 | 检查对象 |
|---|---|---|---|
| `LET` | `0x00` | 向E, S, F写入立即数时 | 若目标为E，检查[CBASE, CLIMIT)；若目标为S/F，检查[DBASE, DLIMIT) |
| `MOV` | `0x01` | 向E, S, F搬运数据时 | 同上 |
| `POP` | `0x17` | 弹栈到E, S, F时 | 检查新值是否在区间内 |
| `SFA` | `0x18` | 修改S时 | 检查S是否在区间内 |
| `RER` | `0x19` | 修改S时 | 检查S是否在区间内 |
| `JMP`等跳转指令 | `0x23`-`0x2F` | 隐式写入P之前 | 检查E的指是否在[CBASE, CLIMIT) |

| # | 指令 | 格式 | 语义 | 操作码 |
|---|---|---|---|---|
| 64 | `SVC` | `SVC` | 陷入内核。硬件将RIN3的CPL置为0，GIE置为0，且护栏因进入内核态而暂时失效。调用中断`0xFE`。 | `0x40` |
